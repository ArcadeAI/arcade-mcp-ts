...
const app = new MCPApp({
	name: "ModularTools",
	version: "1.0.0",
	instructions: "Demonstrates loading tools from separate modules via addToolsFrom()",
});
...
app.tool(
	"echo_with_logging",
	{
		description: "Echo a message while demonstrating context.log and context.progress",
		parameters: z.object({
			message: z.string().describe("Message to echo"),
			steps: z.number().int().min(1).max(10).default(3).describe("Simulated processing steps"),
		}),
	},
	async (args, context) => {
		...
		for (let i = 1; i <= args.steps; i++) {
			await context.progress.report(i, args.steps, `Processing step ${i}/${args.steps}`);
			// Simulate some work
